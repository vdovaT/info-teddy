...
export function getSwapLink(token0Address, token1Address = null) {
  if (!token1Address) {
    return `https://teddyswap.co/#/swap?inputCurrency=${token0Address}`
  } else {
    return `https://teddyswap.co/#/swap?inputCurrency=${token0Address === '0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c' ? 'BNB' : token0Address
      }&outputCurrency=${token1Address === '0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c' ? 'BNB' : token1Address}`
  }
}

export function getMiningPoolLink(token0Address) {
  return `https://teddyswap.co/#/uni/ETH/${token0Address}`
}

export function getUniswapAppLink(linkVariable) {
  let baseUniswapUrl = 'https://teddyswap.co/#/uni'
  if (!linkVariable) {
